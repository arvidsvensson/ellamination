...
private static final String STATIC_GENERATED = "./static/generated";
    ...
    public String generate(Route route) throws IOException, DocumentException {
        ...
        for (List<Piece> page : pages) {
			document.newPage();
			createPage(document, new DocName(route), route.getName() + " " + pieces.size(), directContent, page, pageNumber++, pages.size(), line);
		}
    ...
    private void createPage(Document document, DocName docName, String name, PdfContentByte directContent, List<Piece> pieces, int pageNumber, int pagesTotal, AtomicInteger line) throws MalformedURLException, DocumentException, IOException {
		   createHeader(document, docName);        
		   createBody(document, directContent, pieces, line);
		   createFooter(document, directContent, name, pageNumber, pagesTotal);    	
    }
        
	private void createHeader(Document document, DocName docName) throws DocumentException, MalformedURLException, IOException {
		...
		String date = new SimpleDateFormat("dd.MM.yyyy").format(docName.date);
		
		cell.setPhrase(new Phrase(date, font));
		table.addCell(cell);
		cell.setPhrase(new Phrase("Service Center:", font));
		table.addCell(cell);
		cell.setPhrase(new Phrase(docName.service, font));
		table.addCell(cell);
		cell.setPhrase(new Phrase("Courier Route ID:", font));
		table.addCell(cell);
		cell.setPhrase(new Phrase(docName.routeId, font));
		table.addCell(cell);
